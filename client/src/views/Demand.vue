...
export default {
  ...
  setup() {
    ...
    const getChangeColor = (forecast) => {
      ...
      if (changePercent <= 2) {
        return 'var(--chart-shipped)' // Blue for stable
      }

      if (change > 0) return 'var(--chart-delivered)' // Green for increasing
      if (change < 0) return 'var(--chart-backordered)' // Red for decreasing
      return 'var(--chart-shipped)' // Blue for no change
    }
...
/* The left accent bar carries the trend direction, so it uses the status colours
   directly rather than the muted -bg variants used for badges. */
.increasing-card {
